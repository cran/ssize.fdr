...
ssize.oneSamp<-function(delta,sigma,fdr=0.05,power=0.8,pi0=0.95,maxN=35,side="two-sided"){
	ds<-delta/sigma
	N<-maxN
	a<-fdr

	if(side=="two-sided"){
		TSfun2<-function(c,a,p,n,t){
			r<-a*(1-p)/((1-a)*p)
			dif<-abs((2*pt(q=-c,df=n-1)/(1-pt(q=c,df=n-1,ncp=t))-r))
			return(dif)
		}
	}
	...
	if(side=="upper"){
		TSfun2<-function(c,a,p,n,t){
			r<-a*(1-p)/((1-a)*p)
			dif<-abs((pt(q=-c,df=n-1)/(1-pt(q=c,df=n-1,ncp=t))-r))
			return(dif)
		}
	}
	...
	if(side=="lower"){
		TSfun2<-function(c,a,p,n,t){
			r<-a*(1-p)/((1-a)*p)
			dif<-abs((pt(q=-c,df=n-1)/pt(q=-c,df=n-1,ncp=t)-r))
			return(dif)
		}
	}
	...
	for(i in 1:length(pi0)){
		...
		for(n in 2:N){
			...
			t<-ds/sqrt(1/ni)
			up<-20
			ci<-optimize(f=TSfun2,interval=c(0,up),a=a,p=p,n=n,t=t)$min
			if(abs(ci-up)>=1){
				if(side=="two-sided"|side=="upper"){pwr.new<-1-pt(q=ci,df=ni-1,ncp=t)}
				if(side=="lower"){pwr.new<-pt(q=-ci,df=ni-1,ncp=t)}
			}
